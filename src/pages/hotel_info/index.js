...
const HotelInfoPage = () => {
    ...
    const handleNext = () => {
        setCurrIndex((prevIndex) => (prevIndex + 1) % rooms.length); // Chuyển sang phần tử tiếp theo
    };
    const handlePrevious = () => {
        setCurrIndex((prevIndex) => (prevIndex - 1 + rooms.length) % rooms.length); // Chuyển về phần tử trước đó
    };
    ...
    return (
        <div className="max-w-[1100px] mx-auto">
            <SlideShow images={rooms[currIndex]?.images || []} />
            <div className="bg-blue-300 w-full rounded-md shadow-sm">
                <div className="flex justify-between items-center bg-white rounded-md shadow-md p-2 w-full max-w-[1100px]">
                    <div className="flex items-center gap-4">
                        <div>
                            <h3 className="text-3xl font-sans text-gray-800"><b>{ rooms[currIndex].name }</b></h3>
                            <div className="flex items-center gap-2">
                                <span className="bg-blue-100 text-blue-500 px-2 py-1 text-sm rounded">{ rooms[currIndex].type }</span>
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-lg font-bold text-red-500">{ rooms[currIndex].price }</p>
                ...
                <div className="flex items-center bg-blue-100 p-2 shadow-md w-full max-w-[1100px]">
                    <p className="ml-4 text-blue-900 text-sm">
                        Dừng khoảng chừng là 2 giây! Chỉ còn <span className="font-bold text-blue-700">một số phòng</span> có giá thấp nhất này!
                    </p>
                </div>    
                ...
                <div className="flex flex-wrap justify-between bg-white shadow-lg p-2 rounded-lg ">
                    ...
                    <div className="w-full bg-blue-50 rounded-md md:w-[33%] p-2">
                        ...
                        <div className="flex items-center mt-2">
                            <p className="ml-2 text-gray-700 text-sm">{ rooms[currIndex].service }</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white shadow-md rounded-md p-3">
                    <h2 className="text-xl font-bold text-gray-800 mb-4">
                        Những phòng còn trống tại { rooms[currIndex].name }
                    </h2>
                    ...
                    <div className="relative">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 py-4">
                            <h1 className="text-white text-lg md:text-2xl font-bold ml-4">
                                Khám phá thêm về {rooms[currIndex].name}
                            </h1>
                        </div>
                    </div>
                    <div className="p-6">
                        <h2 className="text-sm font-bold text-gray-800 mb-4">
                            Mô tả về khách sạn {rooms[currIndex].name}
                        </h2>
                        <p className="text-gray-700 text-xs leading-relaxed">
                            { rooms[currIndex].comment }
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-white shadow-md rounded-lg p-5">
                    <h2 className="text-xl font-bold text-gray-800 mb-4">
                        Tất cả những tiện ích tại {rooms[currIndex].name}
                    </h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        {rooms[currIndex].facilities.map((facility, index) => (
                        <div
                            key={index}
